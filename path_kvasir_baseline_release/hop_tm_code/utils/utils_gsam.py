...
def get_dataset(dataset, data_path, batch_size=1, args=None):
    ...
    if dataset == 'PathMnist':
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(),
                                            ...
                                            transforms.Resize(im_size, interpolation=transforms.InterpolationMode.BICUBIC),
                                            transforms.CenterCrop(im_size)])
        dst_train = PathMNIST(split='train', download=True)  # no augmentation
        ...
        channel = 3  # 3 channels for better feature extraction
        im_size = (112, 112)
        num_classes = 4
        ...
        dst_test = datasets.ImageFolder(os.path.join(data_path, "test"), transform=transform)
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=64, shuffle=False, num_workers=2)

    return channel, im_size, num_classes, mean, std, dst_train, dst_test, testloader, loader_train_dict, class_map, class_map_inv
