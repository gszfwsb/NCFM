...
from NCFM.datadam_attention import datadam_guided_match_loss
...
class Condenser:
    ...
    def condense(
        ...
        if getattr(args, "dam_enabled", False):
            inner_loss_fn = datadam_guided_match_loss
            if args.rank == 0:
                args.logger(
                    "DataDAM attention guidance enabled: "
                    f"objective={getattr(args, 'dam_objective', 'ncfm_attention')}, "
                    f"layers={getattr(args, 'dam_attention_layers', 'all')}, "
                    f"feature_weight={getattr(args, 'dam_feature_weight', 1.0)}, "
                    f"attention_weight={getattr(args, 'dam_attention_weight', 10.0)}, "
                    f"p={getattr(args, 'dam_attention_p', 2)}, "
                    f"norm={getattr(args, 'dam_attention_norm', 'l2')}"
                )
        else:
            inner_loss_fn = match_loss if args.depth <= 5 else mutil_layer_match_loss
        if getattr(args, "use_discrepancy_attention_ncfd", False) and args.rank == 0:
            args.logger(
                "Discrepancy-guided spatial attention NCFD enabled: "
                f"lambda={getattr(args, 'lambda_discrepancy_attention_ncfd', 0.1)}, "
                f"layers={getattr(args, 'discrepancy_attention_layers', 'last')}, "
                f"mode={getattr(args, 'discrepancy_attention_mode', 'softmax')}, "
                f"tau={getattr(args, 'discrepancy_attention_tau', 1.0)}, "
                f"num_freqs={getattr(args, 'discrepancy_attention_num_freqs', 'auto')}, "
                f"loss_scale={getattr(args, 'discrepancy_attention_loss_scale', 300.0)}"
            )
        if getattr(args, "use_dr_ltm_ncfd", False) and args.rank == 0:
            args.logger(
                "DR-LTM CVaR local token NCFD enabled: "
                f"lambda={getattr(args, 'lambda_dr_ltm_ncfd', 0.1)}, "
                f"alpha={getattr(args, 'dr_ltm_alpha', 0.25)}, "
                f"layers={getattr(args, 'dr_ltm_layers', 'last')}, "
                f"mode={getattr(args, 'dr_ltm_mode', 'topk')}, "
                f"num_freqs={getattr(args, 'dr_ltm_num_freqs', 'auto')}, "
                f"loss_scale={getattr(args, 'dr_ltm_loss_scale', 300.0)}"
            )
        if getattr(args, "use_feature_map_token_ncfd", False) and args.rank == 0:
            args.logger(
                "Feature-map token NCFD enabled: "
                f"lambda={getattr(args, 'lambda_feature_map_token_ncfd', 0.1)}, "
                f"layers={getattr(args, 'feature_map_token_layers', 'last')}, "
                f"num_freqs={getattr(args, 'feature_map_token_num_freqs', 'auto')}, "
                f"loss_scale={getattr(args, 'feature_map_token_loss_scale', 300.0)}, "
                f"pos_weight={getattr(args, 'feature_map_token_pos_weight', 0.0)}, "
                f"pool={getattr(args, 'feature_map_token_pool', False)}"
            )
        if args.sampling_net:
            scheduler_sampling_net = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optim_sampling_net, mode="min", factor=0.5, patience=500
        )
        else:
        ...
        for it in range(args.niter):
            ...
            if it % args.it_log == 0 and args.rank == 0:
                ...
                dam_msg = ""
                if getattr(args, "dam_enabled", False) and hasattr(args, "_dam_last_attention_loss"):
                    dam_msg = (
                        f" dam-feature-loss: {getattr(args, '_dam_last_feature_loss', 0.0):.4f} "
                        f"dam-attn-loss: {getattr(args, '_dam_last_attention_loss', 0.0):.4f} "
                        f"dam-total-loss: {getattr(args, '_dam_last_total_loss', 0.0):.4f}"
                    )
                lp_msg = ""
                if getattr(args, "use_local_patch_feature_ncfd", False) and hasattr(args, "_lp_last_local_loss"):
                    lp_msg = (
                        f" lp-global-loss: {getattr(args, '_lp_last_global_loss', 0.0):.4f} "
                        f"lp-local-loss: {getattr(args, '_lp_last_local_loss', 0.0):.4f} "
                        f"lp-weighted-local-loss: {getattr(args, '_lp_last_weighted_local_loss', 0.0):.4f} "
                        f"lp-total-loss: {getattr(args, '_lp_last_total_loss', 0.0):.4f} "
                        f"lp-encoder-index: {getattr(args, '_local_patch_last_encoder_index', '')}"
                    )
                dgsa_msg = ""
                if getattr(args, "use_discrepancy_attention_ncfd", False) and hasattr(args, "_dgsa_last_att_loss"):
                    dgsa_msg = (
                        f" dgsa-global-loss: {getattr(args, '_dgsa_last_global_loss', 0.0):.4f} "
                        f"dgsa-att-loss: {getattr(args, '_dgsa_last_att_loss', 0.0):.4f} "
                        f"dgsa-weighted-att-loss: {getattr(args, '_dgsa_last_weighted_att_loss', 0.0):.4f} "
                        f"dgsa-total-loss: {getattr(args, '_dgsa_last_total_loss', 0.0):.4f} "
                        f"dgsa-entropy: {getattr(args, '_dgsa_last_entropy', 0.0):.4f} "
                        f"dgsa-maxw: {getattr(args, '_dgsa_last_max_weight', 0.0):.4f}"
                    )
                dr_ltm_msg = ""
                if getattr(args, "use_dr_ltm_ncfd", False) and hasattr(args, "_dr_ltm_last_loss"):
                    dr_ltm_msg = (
                        f" drltm-global-loss: {getattr(args, '_dr_ltm_last_global_loss', 0.0):.4f} "
                        f"drltm-loss: {getattr(args, '_dr_ltm_last_loss', 0.0):.4f} "
                        f"drltm-weighted-loss: {getattr(args, '_dr_ltm_last_weighted_loss', 0.0):.4f} "
                        f"drltm-total-loss: {getattr(args, '_dr_ltm_last_total_loss', 0.0):.4f} "
                        f"drltm-mean-d: {getattr(args, '_dr_ltm_last_mean', 0.0):.4f} "
                        f"drltm-max-d: {getattr(args, '_dr_ltm_last_max', 0.0):.4f} "
                        f"drltm-p90-d: {getattr(args, '_dr_ltm_last_p90', 0.0):.4f} "
                        f"drltm-cvar-d: {getattr(args, '_dr_ltm_last_cvar', 0.0):.4f} "
                        f"drltm-tail-ratio: {getattr(args, '_dr_ltm_last_tail_ratio', 0.0):.4f} "
                        f"drltm-cv: {getattr(args, '_dr_ltm_last_cv', 0.0):.4f} "
                        f"drltm-k: {getattr(args, '_dr_ltm_last_k', 0.0):.1f}"
                    )
                fmt_msg = ""
                if getattr(args, "use_feature_map_token_ncfd", False) and hasattr(args, "_fmt_last_token_loss"):
                    fmt_msg = (
                        f" fmt-global-loss: {getattr(args, '_fmt_last_global_loss', 0.0):.4f} "
                        f"fmt-token-loss: {getattr(args, '_fmt_last_token_loss', 0.0):.4f} "
                        f"fmt-weighted-token-loss: {getattr(args, '_fmt_last_weighted_token_loss', 0.0):.4f} "
                        f"fmt-total-loss: {getattr(args, '_fmt_last_total_loss', 0.0):.4f} "
                        f"fmt-raw-loss: {getattr(args, '_fmt_last_raw_loss', 0.0):.4f} "
                        f"fmt-token-count: {getattr(args, '_fmt_last_token_count', 0.0):.1f} "
                        f"fmt-dim: {getattr(args, '_fmt_last_dim', 0.0):.1f}"
                    )
                if args.iter_calib > 0:
                    args.logger(
                        f"\n{get_time()} (Iter {it:3d}) "
                        ...
                        f"{dam_msg} {lp_msg} {dgsa_msg} {dr_ltm_msg} {fmt_msg} "
