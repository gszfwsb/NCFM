        ...
        if (
            ...
            if is_best:
                ...
                best_metrics = {
                    ...
                    "ipc": args.ipc,
                    "epoch": epoch,
                    "epochs": args.evaluation_epochs,
                    "is_best": True,
                    "checkpoint_path": checkpoint_path,
                    **val_metrics,
                }
                if checkpoint_path and args.rank == 0:
                    os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
                    torch.save(model.module.state_dict(), checkpoint_path)
                    if logger is not None:
                        logger(f"Saved best evaluator checkpoint: {checkpoint_path}")
            metrics_record = {
                "phase": "synthetic_eval",
                "dataset": args.dataset,
                "ipc": args.ipc,
            ...
            metrics_path = getattr(args, "eval_metrics_path", None)
            if metrics_path and args.rank == 0:
                append_jsonl(metrics_path, metrics_record)
                if is_best:
                    base, _ = os.path.splitext(metrics_path)
                    write_json(base + "_best.json", best_metrics)
            if logger is not None and args.rank == 0:
                ...
                logger(f"Current metrics: {format_metrics(val_metrics)}")
                logger(
