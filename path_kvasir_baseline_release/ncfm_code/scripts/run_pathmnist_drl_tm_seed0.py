...
JOBS = [
    dict(group="DRLTM_lam01_a100_L1_nf256", lam=0.1, alpha=1.00, layers="[1]", nf=256, mode="topk"),
    dict(group="DRLTM_lam02_a100_L1_nf256", lam=0.2, alpha=1.00, layers="[1]", nf=256, mode="topk"),
    dict(group="DRLTM_lam02_a050_L1_nf256", lam=0.2, alpha=0.50, layers="[1]", nf=256, mode="topk"),
    dict(group="DRLTM_lam02_a025_L1_nf256", lam=0.2, alpha=0.25, layers="[1]", nf=256, mode="topk"),
    dict(group="DRLTM_lam02_a010_L1_nf256", lam=0.2, alpha=0.10, layers="[1]", nf=256, mode="topk"),
    dict(group="DRLTM_lam03_a025_L1_nf256", lam=0.3, alpha=0.25, layers="[1]", nf=256, mode="topk"),
]
...
def make_config(args, job):
    ...
    return {
        ...
        "condense": {
            ...
            "lambda_dr_ltm_ncfd": float(job["lam"]),
            "dr_ltm_layers": job["layers"],
            "dr_ltm_num_freqs": int(job["nf"]),
            "dr_ltm_alpha": float(job["alpha"]),
            "dr_ltm_mode": job["mode"],
            "dr_ltm_loss_scale": float(args.loss_scale),
...
def save_summary(exp_root, rows):
    ...
    (report_dir / "pathmnist_drl_tm_summary.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
    fields = ["dataset", "group", "lambda", "alpha", "layers", "nf", "mode", "acc_percent", "auc_macro_ovr", "macro_f1", "balanced_acc", "condensed_path", "checkpoint_path"]
    with (report_dir / "pathmnist_drl_tm_summary.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fields, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(rows)
    lines = ["| Group | lambda | alpha | mode | ACC | AUC | Macro-F1 | BACC |", "|---|---:|---:|---|---:|---:|---:|---:|"]
    for r in rows:
        lines.append(f"| {r.get('group')} | {r.get('lambda')} | {r.get('alpha')} | {r.get('mode')} | {r.get('acc_percent')} | {r.get('auc_macro_ovr')} | {r.get('macro_f1')} | {r.get('balanced_acc')} |")
    (report_dir / "pathmnist_drl_tm_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def select_jobs(group_spec):
    spec = str(group_spec or "all").strip()
    if spec == "all":
        return JOBS
    if spec == "even":
        return [job for idx, job in enumerate(JOBS) if idx % 2 == 0]
    if spec == "odd":
        return [job for idx, job in enumerate(JOBS) if idx % 2 == 1]
    wanted = {item.strip() for item in spec.split(",") if item.strip()}
    selected = [job for job in JOBS if job["group"] in wanted]
    missing = wanted - {job["group"] for job in selected}
    if missing:
...
def main():
    ...
    parser.add_argument("--groups", default="all", help="Comma-separated group names, even, odd, or all.")
