...
class LossPlotter:
    ...
    def plot_and_save_loss_curve(self):
        ...
        if len(os.path.abspath(file_name)) > 240:
            digest = hashlib.sha1(self.filename_pattern.encode("utf-8")).hexdigest()[:10]
            file_name = os.path.join(self.save_path, f"loss_acc_{digest}.png")
