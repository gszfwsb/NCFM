...
def set_iteration_parameters(niter, debug):
    ...
    if niter not in it_save:
        it_save.append(niter)
    it_log = 1 if debug else 20
    return it_save, it_log


def set_Pretrain_Directory(pretrain_dir, dataset, depth):
    prefix = "" if os.path.isabs(pretrain_dir) else "."
    if dataset.lower() == "imagenet":
        pretrain_dir = os.path.join(prefix, pretrain_dir, dataset, f"ResNet-{depth}")
    else:
        pretrain_dir = os.path.join(prefix, pretrain_dir, dataset)
    return pretrain_dir
